...
exports.unlinkStudentLine = unlinkStudentLine;
...
function normalizeSubjectCode(text) {
...
async function handleEvent(event, client) {
    ...
    if (event.type === 'message' && event.message.type === 'text') {
        ...
        if (isGroupMessage && groupId && /^bind\s*:/i.test(rawText)) {
            ...
            if (!subjectCode) {
                await replyText(client, event.replyToken, 'รูปแบบคำสั่งไม่ถูกต้อง\nตัวอย่าง: bind: IT101');
                return;
            }
            ...
            if (subjectSnap.empty) {
                await replyText(client, event.replyToken, `ไม่พบวิชารหัส "${subjectCode}" ในระบบ\nกรุณาตรวจสอบรหัสวิชาอีกครั้ง`);
                return;
            }
            if (subjectSnap.docs.length > 1) {
                await replyText(client, event.replyToken, `พบรหัสวิชา "${subjectCode}" มากกว่า 1 รายการ\nกรุณาใช้รหัสวิชาที่ไม่ซ้ำกันก่อน bind กลุ่ม`);
                return;
            }
        ...
        console.log(`[lineWebhook] incoming text="${rawText}" normalizedStudentId="${normalizedStudentId}" user=${lineUserId.slice(0, 6)}...`);
        // Ignore messages that don't look like a student ID (digits only, 3-15 chars)
        if (!/^\d{3,15}$/.test(normalizedStudentId)) {
            await replyText(client, event.replyToken, 'กรุณาส่งเฉพาะรหัสนักเรียน (ตัวเลขเท่านั้น) เช่น "65309010001"');
            return;
        }
        ...
        if (!studentDoc) {
            ...
            await replyText(client, event.replyToken, `ไม่พบรหัสนักเรียน "${studentId}" ในระบบ\nกรุณาตรวจสอบรหัสของคุณหรือติดต่ออาจารย์`);
            return;
        }
        const existingLineUserId = studentDoc.get('line_user_id');
        // ── Prevent hijacking another student's account ─────────────────────────
        if (existingLineUserId && existingLineUserId !== lineUserId) {
            await replyText(client, event.replyToken, 'รหัสนักเรียนนี้ถูกเชื่อมต่อกับบัญชี LINE อื่นแล้ว\nกรุณาติดต่ออาจารย์เพื่อรีเซ็ตการเชื่อมต่อ');
            return;
        }
        // ── Already linked to this LINE account ──────────────────────────────────
        if (existingLineUserId === lineUserId) {
            await replyText(client, event.replyToken, `บัญชีของคุณถูกเชื่อมต่อกับรหัสนักเรียน "${studentId}" แล้ว ✅\nคุณจะได้รับการแจ้งเตือนงานผ่าน LINE นี้`);
            return;
        }
        // ── Save line_user_id ─────────────────────────────────────────────────────
        ...
        const studentName = studentDoc.get('name');
        await replyText(client, event.replyToken, [
            `เชื่อมต่อสำเร็จ! ✅`,
            ``,
...
exports.lineWebhook = (0, https_1.onRequest)({
    ...
    if (!signature ||
        !(0, bot_sdk_1.validateSignature)(rawBody, LINE_CHANNEL_SECRET.value(), signature)) {
    ...
    const results = await Promise.allSettled(req.body.events.map((event) => handleEvent(event, client)));
...
// --------------------------------------------------------------------------
async function unlinkStudentLine(studentDocId) {
